...
(function() {
    ...
    // --- State Management ---
    let savedItems = JSON.parse(GM_getValue('wtr_saved_items', '[]')); 
    let matchMode = GM_getValue('wtr_match_mode', 'broad'); 
    let allApiTags = [];
    let uiHiddenByRoute = false; 
    
    // Core targets to latch onto
    ...
    function isExcludedPage() {
        ...
        if (/\/novel\/.*\/chapter-/i.test(path)) return true;
        if (path.includes('/library') || path.includes('/news')) return true;
        if (path.includes('/profile/') && !path.includes('/profile/vote-serie')) return true;
        return false;
    }

    // --- Inject Lightweight, High-Contrast Styles ---
    GM_addStyle(`
        :root {
    ...
    panel.innerHTML = `
        <div id="dd-header">
            <div class="dd-title-wrap">${wtrSvg}<h2>D. DESTROYER</h2></div>
    ...
    const selectMatch = document.getElementById('dd-match');
    const inputField = document.getElementById('dd-input');
    const autocompleteBox = document.getElementById('dd-autocomplete');
    const blocklistEl = document.getElementById('dd-blocklist');
    const countEl = document.getElementById('dd-count');

    selectMatch.value = matchMode;

    // --- Core Logic ---
    function updateStorage() {
        GM_setValue('wtr_saved_items', JSON.stringify(savedItems));
        GM_setValue('wtr_match_mode', matchMode);
    }

    function getNextBuildId() {
        const nextDataScript = document.getElementById('__NEXT_DATA__');
        if (nextDataScript) { try { return JSON.parse(nextDataScript.textContent).buildId; } catch (e) {} }
        return 'B_iTIc03bagM1u-Uo_553';
    }

    // The Annihilator
    function executeDestroyer() {
        if (isExcludedPage()) {
            if (!uiHiddenByRoute) {
                launcher.style.display = 'none';
                panel.style.display = 'none';
                uiHiddenByRoute = true;
            }
            return; 
        } else {
            if (uiHiddenByRoute) {
                launcher.style.display = 'flex';
                uiHiddenByRoute = false;
            }
        }

        if (savedItems.length === 0) return;

        const blockWords = savedItems.map(item => item.label.toLowerCase());
        const elements = document.querySelectorAll(TARGET_SELECTORS);

        elements.forEach(el => {
            let container = el;
            const cardWrapper = el.closest('.card');
            
            if (cardWrapper) {
                const itemsInCard = cardWrapper.querySelectorAll('.serie-item, .list-item, .rank-item, .rec-item, .recent-item');
                if (itemsInCard.length === 1) {
            ...
            if (titleNode) {
                textToCheck = (titleNode.getAttribute('title') || titleNode.textContent || '').toLowerCase();
            } else if (container.hasAttribute('title')) {
                textToCheck = container.getAttribute('title').toLowerCase();
            }

            const metaTags = Array.from(container.querySelectorAll('.genre, .tag')).map(t => t.textContent.toLowerCase());
            
            let shouldDestroy = false;

            if (matchMode === 'broad') {
                shouldDestroy = blockWords.some(word => textToCheck.includes(word) || metaTags.includes(word));
            } else {
                shouldDestroy = blockWords.some(word => metaTags.includes(word));
            }
    ...
    // SPA Routing & Infinite Scroll Fixes
    const originalPushState = history.pushState;
    const originalReplaceState = history.replaceState;
    history.pushState = function() { originalPushState.apply(this, arguments); setTimeout(executeDestroyer, 100); };
    history.replaceState = function() { originalReplaceState.apply(this, arguments); setTimeout(executeDestroyer, 100); };
    window.addEventListener('popstate', () => setTimeout(executeDestroyer, 100));

    const observer = new MutationObserver(() => {
        clearTimeout(window.ddDestroyTimeout);
        window.ddDestroyTimeout = setTimeout(executeDestroyer, 250);
    });
    ...
    function renderList() {
        ...
        savedItems.forEach(item => {
            ...
            let icon = '*';
            if (item.type === 'genre') icon = 'G';
            if (item.type === 'tag') icon = '#';

            pill.innerHTML = `<span class="dd-pill-icon">${icon}</span> <span>${item.label}</span> <button class="dd-remove">×</button>`;
            
            pill.querySelector('.dd-remove').addEventListener('click', () => {
                savedItems = savedItems.filter(i => i.id !== item.id);
                updateStorage(); renderList(); executeDestroyer(); 
            });
            blocklistEl.appendChild(pill);
        });

        countEl.textContent = savedItems.length;
        if (savedItems.length === 0) blocklistEl.innerHTML = `<div class="dd-empty">No targets acquired.</div>`;
    }
    ...
    function getSuggestions(query) {
        ...
        // Extract currently saved labels so we can exclude them from the suggestions
        const savedLabels = savedItems.map(item => item.label.toLowerCase());
        
        // Map static genres (filtering out those already in the blocklist)
        const genreMatches = WTR_GENRES
            .filter(g => g.toLowerCase().includes(lowerQuery) && !savedLabels.includes(g.toLowerCase()))
            .map(g => ({ label: g, type: 'genre', id: 'g_' + g.toLowerCase() }));
            
        // Map dynamic API tags (filtering out those already in the blocklist)
        const tagMatches = allApiTags
            .filter(t => t.label.toLowerCase().includes(lowerQuery) && !savedLabels.includes(t.label.toLowerCase()))
            .map(t => ({ label: t.label, type: 'tag', id: t.value }));
            
        // Combine and limit to top 15 matches to keep UI clean
    ...
    function renderAutocomplete() {
        ...
        suggestions.forEach(sug => {
            ...
            item.innerHTML = `<span>${sug.label}</span> <span class="dd-ac-type">${sug.type}</span>`;
            
            item.addEventListener('click', () => {
                inputField.value = sug.label;
                addWord(); // Instantly add it
                autocompleteBox.style.display = 'none';
            });
    ...
    document.addEventListener('click', (e) => {
        if (e.target !== inputField && e.target !== autocompleteBox) {
            autocompleteBox.style.display = 'none';
        }
    });
    ...
    function addWord() {
        ...
        if (savedItems.some(i => i.label.toLowerCase() === val.toLowerCase())) {
            inputField.value = ''; 
            autocompleteBox.style.display = 'none';
            return; 
        }
        ...
        if (WTR_GENRES.includes(val.toLowerCase())) {
            type = 'genre';
            id = 'g_' + val.toLowerCase();
        } else {
            const foundTag = allApiTags.find(t => t.label.toLowerCase() === val.toLowerCase() || t.value.toString() === val);
            if (foundTag) {
                type = 'tag';
        ...
        savedItems.push({ id, label, type });
        updateStorage(); renderList();
        inputField.value = '';
        autocompleteBox.style.display = 'none';
    }
    ...
    selectMatch.addEventListener('change', (e) => { matchMode = e.target.value; updateStorage(); });
    
    // Autocomplete listeners
    inputField.addEventListener('input', renderAutocomplete);
    inputField.addEventListener('focus', renderAutocomplete);
    
    document.getElementById('dd-add').addEventListener('click', addWord);
    inputField.addEventListener('keypress', (e) => { if (e.key === 'Enter') addWord(); });

    // Custom Modal Listeners
    const modalOverlayEl = document.getElementById('dd-modal-overlay');
    
    document.getElementById('dd-purge').addEventListener('click', () => {
        modalOverlayEl.style.display = 'flex';
    });
    
    document.getElementById('dd-modal-cancel').addEventListener('click', () => {
        modalOverlayEl.style.display = 'none';
    });
    
    document.getElementById('dd-modal-confirm').addEventListener('click', () => {
        savedItems = []; 
        updateStorage(); 
        renderList(); 
        executeDestroyer();
    ...
    async function init() {
        renderList();
        try {
            const res = await fetch(`https://wtr-lab.com/_next/data/${getNextBuildId()}/en/novel-finder.json?locale=en`);
            if (res.ok) {
                const json = await res.json();
                let ungrouped = null;
                const findTags = (obj) => {
                    if (!obj || typeof obj !== 'object') return null;
                    if (obj.tags && obj.tags.ungrouped && Array.isArray(obj.tags.ungrouped)) return obj.tags.ungrouped;
                    for (let key in obj) { let r = findTags(obj[key]); if (r) return r; } return null;
                };
                ungrouped = findTags(json);
                if (ungrouped) allApiTags = ungrouped;
            }
        } catch (e) { console.log("Delulu Destroyer: Running without API tag autocompletion."); }
